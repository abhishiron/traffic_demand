...
src/train.py
------------
Time-based holdout validation + LightGBM training.

Validation strategy (mirrors train→test gap):
    fit  : day 48  00:00 – 21:45  (slots 0 – 1305 min)
...
from features import CATEGORICALS, build_features
...
def _train_eval(
    ...
    """
    Train one LGBM model and return (model, holdout_r2, best_iteration).

    The eval set target is transformed if use_logit=True so that early
    stopping monitors the right scale.  R² is always computed on raw [0,1]
    demand after inverting the transform.
    """
    ...
    return model, r2, int(model.best_iteration_)
...
def main() -> None:
    ...
    # fit  : day 48, slots 00:00 – 21:45  (minutes 0 – 1305)
    # val  : day 48, slots 22:00 – 23:45  (minutes 1320 – 1425)
    #       + day 49, all slots            (minutes 0 – 120)
    ...
    # ── Build features (encoders fit on fit portion only) ─────────────────
    print("\nBuilding features ...")
    ...
    cat_idxs = [
        list(X_fit.columns).index(c)
        for c in CATEGORICALS
        if c in X_fit.columns
    ]
    print(f"  feature cols : {list(X_fit.columns)}")
    print(f"  cat indices  : {cat_idxs}  → {[list(X_fit.columns)[i] for i in cat_idxs]}")

    # ── Holdout: raw demand ───────────────────────────────────────────────
    print("\n" + "-" * 40)
    print("  [1/2] Training on RAW demand ...")
    m_raw, r2_raw, iter_raw = _train_eval(
        X_fit, y_fit, X_val, y_val, use_logit=False, cat_idxs=cat_idxs
    )
    print(f"  Holdout R²: {r2_raw:.6f}   score ≈ {max(0, 100*r2_raw):.2f}   "
          f"best_iter: {iter_raw}")

    # ── Holdout: logit-transformed demand ─────────────────────────────────
    print("\n" + "-" * 40)
    print("  [2/2] Training on LOGIT(demand) ...")
    m_logit, r2_logit, iter_logit = _train_eval(
        X_fit, y_fit, X_val, y_val, use_logit=True, cat_idxs=cat_idxs
    )
    print(f"  Holdout R²: {r2_logit:.6f}   score ≈ {max(0, 100*r2_logit):.2f}   "
    ...
    cat_idxs_full = [
        list(X_full.columns).index(c)
        for c in CATEGORICALS
        if c in X_full.columns
    ]

    final_model = lgb.LGBMRegressor(**{**_base_params(), "n_estimators": final_n})
    final_model.fit(X_full, y_full_tr, categorical_feature=cat_idxs_full)
    ...
    fi = pd.concat([fi_split, fi_gain], axis=1).sort_values("gain", ascending=False)
